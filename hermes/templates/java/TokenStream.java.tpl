...
class TokenStream extends ArrayList<Terminal> {
  ...
  public Terminal expect(int expecting) throws SyntaxError {
    ...
    if (current.getId() != expecting) {
      ...
      formatter.format("Unexpected symbol when parsing %s.  Expected %s, got %s", stack[1].getMethodName(), this.terminals.get(expecting), current != null ? current.toString() : "<end of stream>");
