...
class {{prefix}}ExpressionParser_{{exprGrammar.nonterminal.string.lower()}} extends ExpressionParser {

  private LL1Parser parent;
  private HashMap<Integer, Integer> infixBp, prefixBp;

  {{prefix}}ExpressionParser_{{exprGrammar.nonterminal.string.lower()}}(LL1Parser parent) {
    this.parent = parent;
    ...
    this.infixBp.add({{terminal_id}}, {{binding_power}});
    {% end %}

    {% for terminal_id, binding_power in exprGrammar.prefix.items() %}
    this.prefixBp.add({{terminal_id}}, {{binding_power}});
    {% endfor %}
  }

  private int getInfixBp(tokenId) {
    Integer value = this.infixBb.get(new Integer(tokenId));
    if ( value )
      return value.toInteger();
    return 0;
  }

  private int getPrefixBp(tokenId) {
    Integer value = this.prefixBb.get(new Integer(tokenId));
    if ( value )
      return value.toInteger();
    return 0;
  
  private int getCurrentToken() {
    return this.parent.tokens.current();
  }

  private int expect(int token) {
    return this.parent.expect(token);
  }
  ...
  public ParseTree parse(int rbp) {
    ...
    while (this.getCurrentToken() && rbp < getInfixBp(getCurrentToken().getId())) {
      left = this.led(left)
    }
