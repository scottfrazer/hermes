...
class Ast2Test(GrammarTest):
  ...
  def test_codeGeneration(self):
    self.runWithTokens(['identifier','eq','identifier','eq','n','semi','for','lparen','identifier','eq','n','semi','semi','identifier','eq','s','add','n','rparen','lbrace','identifier','eq','n','add','identifier','mul','n','power','s','semi','rbrace','semi','identifier','add','identifier']) \
        .assertParseTree('(start: (_gen0: (statement: (_expr: (_expr: identifier, eq, identifier), eq, n)), (_gen1: semi, (statement: (for: for, lparen, (forsub: (_expr: identifier, eq, n)), semi, (forsub: ), semi, (forsub: (_expr: identifier, eq, (_expr: s, add, n))), rparen, lbrace, (forbody: (statement: (_expr: identifier, eq, (_expr: n, add, (_expr: identifier, mul, (_expr: n, power, s))))), semi, (forbody: )), rbrace)), (_gen1: semi, (statement: (_expr: identifier, add, identifier)), (_gen1: )))))') \
        .assertAst('(Program: statements=[(Assign: rhs=n, lhs=(Assign: rhs=identifier, lhs=identifier)), (For: decl=(Assign: rhs=n, lhs=identifier), body=(Assign: rhs=(Add: rhs=(Multiply: rhs=(Power: rhs=s, lhs=n), lhs=identifier), lhs=n), lhs=identifier), cond=None, iter=(Assign: rhs=(Add: rhs=n, lhs=s), lhs=identifier)), (Add: rhs=identifier, lhs=identifier)])')
